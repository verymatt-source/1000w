...
def is_a_share_trading_time():
    ...
    Actions已设置为 Asia/Shanghai 时区，直接使用 datetime.now()。
    """
    now = datetime.now() 
    hour = now.hour
    minute = now.minute
...
def main():
    ...
    for config in TARGET_STOCKS:
        ...
        if schedule_mode == "MARKET" and not is_market_open:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 跳过 {config['name']} ({code})，当前非交易时间（{schedule_mode} 模式）。")
            continue
        ...
        all_targets.append({
            ...
            "config": config,
            "api_func": get_stock_data_from_sina,
            "schedule_mode": schedule_mode # 【关键】将运行模式带入数据结构
        })
        
    # 组合 CALCULATED_TARGETS
    for config in CALCULATED_TARGETS:
        code = config['code']
        schedule_mode = config.get("update_schedule", "MARKET") # 默认设为 MARKET
        ...
        api_func = get_cb_codes_from_eastmoney if code == "CB/AVG" else get_stock_data_from_sina
        all_targets.append({
            "target_code": code,
            "target_price": target_price,
            "target_note": target_note,
    ...
    # 如果没有需要采集的标的，则结束运行
    if not all_targets:
        print("所有标的均在休市模式下，本次运行无数据采集任务，脚本结束。")
        return
        ...
        <table class="styled-table">
            <thead>
                <tr>
                    ...
                    <th>当前价</th>
                    <th>目标偏离度</th>
                    <th>运行方式</th> <th style="text-align: left;">备注</th>
                </tr>
            </thead>
            <tbody>
                {table_content}
